...
//
            ...
            final File newManDex = internNativeUtilClassDef(
                    mainDex,
                    globalConfig,
                    BuildNativeLib.LABA_NAME);
            //替换为新的dex
            outDexFiles.set(0, newManDex);

            //转移dex
            String dex_pac = apkFolders.getOutRootDir() +File.separator+ "dex_pac";
            for (File file : outDexFiles) {
                File des_pac = new File(dex_pac, file.getName());
//                System.out.println(des_pac);
                if (!des_pac.getParentFile().exists()){des_pac.getParentFile().mkdirs();}
                InputStream inputStream = Files.newInputStream(file.toPath());
                final FileOutputStream outputStream = new FileOutputStream(des_pac);
                FileUtils.copyStream(inputStream, outputStream);
            }
            FileUtils.deleteFile(apkFolders.getTempDexDir());
